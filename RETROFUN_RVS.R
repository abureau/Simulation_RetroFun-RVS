...
sharing.causal.variants.by.fam = function(pedfile, sfs.file, regions.of.interest=NULL){
  sample = read.pedfile(pedfile)
  fam = sample$fam
  fam$affected[is.na(fam$affected)] = 1
  ...
  if(!is.null(regions.of.interest)){
    ...
    #Number of causal variants within the considered region(s)
    genotypes_affected_causal_variants = genotypes_affected[,which(sfs.file$V7==1&sfs.file$V1%in%regions.of.interest)]
  }
  else{
    genotypes_affected_causal_variants = genotypes_affected[,which(sfs.file$V7==1)]
  }
  
  genotypes_affected_causal_variants$pedigree = affected$pedigree
  
  #Number of carriers by causal variants in each family
  agg_causal_variants = aggregate(.~pedigree,genotypes_affected_causal_variants, function(x) sum(x>0))
  ...
  colnames(df_sharing_causal_by_fam) = c("FamID", "N_Shared_Causal_Variants")
  
  df_sharing_causal_by_fam$N_Shared_Causal_Variants=as.numeric(df_sharing_causal_by_fam$N_Shared_Causal_Variants)
  df_sharing_causal_by_fam = df_sharing_causal_by_fam[df_sharing_causal_by_fam$N_Shared_Causal_Variants>0,]
  mean(df_sharing_causal_by_fam$N_Shared_Causal_Variants)
...
df.sharing.causal.variants = data.frame("Mean_Number_of_causal_variants" = c(sharing.causal.global.100.02,
                                                                             sharing.causal.global.100.04,
                                                                             sharing.causal.global.100.06,
                                                                             sharing.causal.CRH.100.02,
                                                                             sharing.causal.CRH.100.04,
                                                                             sharing.causal.CRH.100.06), "Proportion_Causal"=rep(c(rep(0.2,100), rep(0.4,100), rep(0.6,100)),2), "Type" = c(rep("Global",300), rep("CRH",300)))

ggplot(df.sharing.causal.variants, aes(x=Type,y=Mean_Number_of_causal_variants, fill=factor(Proportion_Causal)))+geom_boxplot()+xlab("Regions")+ylab("Mean Number of Causal Variants by family")+labs(fill="Proportion Causal")
