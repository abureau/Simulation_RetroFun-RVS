...
ped_files_OR5_100.02 = list.files("../2causal_OR5_RVS/100causal/", full.names=T, recursive=T)
ped_files_OR5_75.02 = list.files("../2causal_OR5_RVS/75causal/", full.names = T, recursive=T)
ped_files_OR5_50.02 = list.files("../2causal_OR5_RVS/50causal/", full.names = T, recursive = T)

# Test avec un seul réplicat
#pedfile = ped_files_OR5_75[1]
#pedfile = ped_files_OR5_50[1]
#pedfile = ped_files_OR5_100.02[1]

# Boucle sur les réplicats
p100.OR5.02.list = pall100.OR5.02.list = jf100.OR5.02.list = pall_lf100.OR5.02.list = list()

for (r in 1:length(ped_files_OR5_100.02))
#for (r in 1:200)
  {
  cat("r=",r,"\n")
  pedfile = ped_files_OR5_100.02[r]
...
p100.OR5.02.list[[r]] = p.vec
pall100.OR5.02.list[[r]] = pall.vec
pall_lf100.OR5.02.list[[r]] = pall_lf.vec
jf100.OR5.02.list[[r]] = jf
#min(pall_lf.vec)*length(pall_lf.vec)  
}

#save(p100.OR5.02.list,pall100.OR5.02.list,pall_lf100.OR5.02.list,jf100.OR5.02.list,file="OR5_100.02_v2.RData")
