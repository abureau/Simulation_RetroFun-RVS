...
n.unique.config.by.fam.sorted = lapply(n.unique.config.by.fam, function(x) sort(x))
...
prob.sharing.by.famid.sorted = lapply(prob.sharing.by.famid, function(x) x[order(as.numeric(names(x)))])

pvalues = readRDS("pvalues_null_Pairs.RDS")
Z = read.table("Annot_Z_pairs.mat")
...
resample.genos.with.inflated.signal = lapply(index.with.inflated.signal, function(x) {
  print(x)
  replicate(1000, resample.genos.by.fam(pedfiles_agg_null[[x]], n.unique.config.by.fam.sorted ,prob.sharing.by.famid.sorted))
})


bootstrap.burden.with.inflated.signal = lapply(1:length(resample.genos.with.inflated.signal), function(x){
	print(x)
	apply(resample.genos.with.inflated.signal[[x]], 2, function(y){
 		compute.Burden.by.Annot(null_without_consanguinity, y, Z, W = rep(1,510))
	})
})
...
saveRDS(l,"/home/loicm/scratch/bootstrap_inflated_signal_Pairs.RDS")
...
bootstrap.burden.with.highly.inflated.signal = lapply(1:length(resample.genos.with.highly.inflated.signal), function(x){
        print(x)
        apply(resample.genos.with.highly.inflated.signal[[x]], 2, function(y){
                compute.Burden.by.Annot(null_without_consanguinity, y, Z, W = rep(1,510))
        })
})
...
saveRDS(l,"/home/loicm/scratch/bootstrap_highly_inflated_signal_Pairs.RDS")
