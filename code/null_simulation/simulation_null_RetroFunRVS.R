...
simulation_null_RetroFunRVS = function(ped.mat,ped.trio.list,ped.haplo,f,ped.varcut,Z,null_exp_var,nrep)
{
...
# ped.varcut : minimal number of families with a variant
...
pedmat[,seq(8,ncol(pedmat),2)] = ped.haplo[ped.sim.haplo[,2],]
  ...
  Z.tmp = Z[,-which(filter.rep.n.fam(pedfiles_agg_null, variants.by.annot)<=ped.varcut), drop=FALSE]
  
  pvalues_null[[i]] = RetroFunRVS::RetroFun.RVS(null_exp_var, pedfiles_agg_null, Z.tmp,rep(1,ncol(ped.haplo)))
